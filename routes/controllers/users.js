...
const validateCredentials = async (req, res) => {
    ...
    try{
      const login =  await pool.db('sorteoI').collection('users').findOne({ email: datos.email, pass: hashedPassword });
      if (login) {
        ...
        await pool.db('sorteoI').collection('log_login').insertOne({ email: datos.email, role: login.role, date: currentDateTime });
  ...
  const Signup = async (req, res) => {
    ...
    try {
        const userFind = await pool.db('sorteoI').collection('users').findOne({ email: datos.email });
        if (userFind) {
            res.status(409).json({ message: `El usuario con el correo: ${datos.email} ya está creado` });
        } else {
            const newUser = await pool.db('sorteoI').collection('users').insertOne({ email: datos.email, pass: hashedPassword, role: datos.role });
